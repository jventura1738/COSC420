...
extern "C" {
    ...
    extern int c_neq(BigInteger* a, BigInteger* b){
        if((*a) != (*b)){
            return 1;
        }
        return 0;    
    ...
    extern BigInteger* c_mm(BigInteger* a){
        (*a)--;  
        BigInteger rtn_o = (*a);
        return new BigInteger(string(rtn_o));    
    }
    ...
    extern int c_is_perf_sq(BigInteger* a){  
        BigInteger* left = makeBigIntStr("1");
        BigInteger* right = a;
        BigInteger* two = makeBigIntStr("2");
    
        while (c_leeq(left,right)) 
        {
            BigInteger* mid = c_div(c_add(left,right), two);
            //printf("Mid: %s\n", c_str(mid));
            // Check if mid is perfect square
            if (c_eqeq(c_mult(mid,mid), a)) 
            {
